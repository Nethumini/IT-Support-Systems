...
if [ -z "$GOOGLE_API_KEY" ]; then
    ...
    echo ""
    echo "   Steps to fix:"
    echo "   1. Go to: https://github.com/Nethumini/IT-Support-Systems"
...
while [ $GIT_RETRY -lt $GIT_MAX_RETRIES ]; do
    if [ ! -d .git ]; then
        if git clone https://github.com/Nethumini/IT-Support-Systems.git . 2>/dev/null; then
            REBUILD_NEEDED=true
            break
        fi
    else
...
OPENAI_API_KEY=
OPENAI_MODEL=gpt-4
DATABASE_URL=sqlite:///./data/processed/it_support_systems.db
SECRET_KEY=it-support-systems-secret-key-change-in-production
...
if [ "$REBUILD_NEEDED" = true ]; then
    ...
    if docker-compose -f docker-compose.deploy.yml build --no-cache 2>/dev/null; then
        ...
        BACKEND_IMAGE=$(docker images | grep it-support-systems-backend | head -1 | awk '{print $3}')
        FRONTEND_IMAGE=$(docker images | grep it-support-systems-frontend | head -1 | awk '{print $3}')
    ...
    while [ $PULL_RETRY -lt 2 ]; do
        if docker pull $DOCKER_HUB_USERNAME/it-support-systems-backend:latest 2>/dev/null && \
           docker pull $DOCKER_HUB_USERNAME/it-support-systems-frontend:latest 2>/dev/null; then
...
DB_PATH="/app/data/processed/it_support_systems.db"
