...
Verified against commit `f3b61e4`, 20 September 2026.
...
**The application never queries it.** Retrieval runs in memory over the JSON
knowledge base, through `DatasetAnalyzer`. `ingestion_script.py` does build a
Chroma store, but nothing in `backend/app/` opens it — and it was built with
hash-based embeddings, which are not semantic.
...
| 42 (5.3.2) | "converted with the text-embedding-004 model and stored in ChromaDB. At query time, semantic retrieval is filtered by environment and approval status." | "converted with the gemini-embedding-001 model and held in an in-memory index. At query time every approved article is ranked by cosine similarity, with a calibrated threshold and a category-agreement bonus; drafts awaiting review are excluded." |
...
| 46 (5.5.2) | "LangChain supports retrieval and agent composition, while ChromaDB stores document vectors." | "Retrieval and agent composition are implemented directly against the Gemini SDK; article vectors are held in memory and cached per process." |
| 47, 51 | ChromaDB listed among integrated components | "the knowledge retrieval service" |

### Option B — change the code (~half a day)

Point `ingestion_script.py` at real Gemini embeddings and make
`DatasetAnalyzer` query Chroma. Only worth it to keep the metadata-filtered
retrieval claim in 5.3.2.

### LangChain has the same problem

Table 5.2 and 5.5.2 credit LangChain for retrieval and agent workflow. Nothing
in `backend/app/` imports it — the only mention is a comment. Remove it from
the technology table, or mark it as a declared dependency not used in the final
implementation.
...
- [ ] **ChromaDB** — decide Option A or B, rewrite pages 41, 42, 46, 47, 51
